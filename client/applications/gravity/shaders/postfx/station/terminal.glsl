...
void main(void) {
    ...
    float b_warp = 0.9+(0.1*(sin(time+uv.x*60*3.14)));
    float c_warp = 0.9+(0.1*(sin(time+uv.x*90*3.14)));
    float d_warp = 0.9+(0.1*(sin(time+uv.x*120*3.14)));

    vec4 smpl_base = texture(texBuffer,uv_t) * (1.0-(warp*a_warp)) * vec4(1.0*c_warp,1.0*b_warp,1.0*d_warp,1.0-uv.y);
    ...
    if(length(col)>0.5) {
        gl_FragColor = vec4(1.0,1.0,1.0,1.0) * filter_color * mod_color;
    } else {
        gl_FragColor = col + (texture(modBuffer,uv_t)*smpl_base.a) * mod_color;
    }
